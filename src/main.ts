...
export async function run(): Promise<void> {
  try {
    ...
    // Create metrics submitter
    const metricsSubmitter = new MetricsSubmitter(config, meterProvider)

    // Process JUnit XML files using existing tooling
    ...
    // Generate and submit metrics for the combined report
...
const parseOtlpHeaders = (headersInput: string): Record<string, string> => {
  ...
  try {
    // Support both JSON format and comma-separated key=value format
